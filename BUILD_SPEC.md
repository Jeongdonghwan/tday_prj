# 오늘연애 (가칭) — 개발 착수 스펙 (FINAL)

연애 상태 기반 오늘연애 커뮤니티 앱. 안드로이드 + iOS 동시 출시.
...
연애 상태(커플·돌싱·유부)를 가진 사용자들이 **투표형 오늘연애**으로 모이고,
**오늘의 질문**으로 매일 들어오고, **커플 기능**으로 머무는 커뮤니티 앱.

핵심 메커니즘
- **상태칩** = 모든 글·댓글·프로필에 붙는 세그먼트 라벨. 콘텐츠를 가르는 환승역.
- **오늘연애** = 글 + 작성자가 직접 만든 2지선다 투표 + 댓글. 캡쳐·바이럴 유도.
...
3. **오늘의 질문** (`(tabs)/daily`)
   - 오늘 질문 1개, 답변 입력
   - 연속 답변 스트릭 표시
   - 커플 연결 시: 둘 다 답하면 서로 공개. 답 갈리면 "오늘연애 올리기" 버튼
...
POST   /daily/answer
POST   /daily/{question_id}/to-post   답 갈린 질문 → 오늘연애 글로 전환
