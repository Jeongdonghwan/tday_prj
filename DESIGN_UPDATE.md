# 썰전 디자인 업데이트 스펙 (DESIGN_UPDATE.md)

거의 완성된 썰전 앱(React Native + Expo)에 적용할 디자인 개선 3종.
...
기존 썰전 피드 카드를 아래 구조로 교체 (토큰은 기존 것 사용).
...
위치: 썰전 피드(커뮤니티 메인) 리스트의 헤더 컴포넌트로 최상단 고정. **활성 질문이 있을 때만 렌더** (없으면 카드 자체 미노출).
